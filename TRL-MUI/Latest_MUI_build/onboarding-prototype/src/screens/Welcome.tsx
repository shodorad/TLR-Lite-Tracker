...
export default function Welcome({ next, goTo, onEnterApp }: WelcomeProps) {
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', pt: '16px' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '7px', py: '18px' }}>
        {Array.from({ length: TOTAL_SLIDES }).map((_, i) => (
          <button
            ...
            onClick={() => goToSlide(i)}
            style={{ background: 'none', border: 'none', padding: '8px 2px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
          >
            <motion.div
              animate={{
                width: i === slide ? 24 : 6,
      ...
      {/* ── CTA section ── */}
      <Box sx={{ px: '24px', pb: '32px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
